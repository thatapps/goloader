...
import com.becapps.easydownloader.ShareActivity;
import com.becapps.easydownloader.YTD;
import com.becapps.easydownloader.ffmpeg.FfmpegController;
import com.becapps.easydownloader.ffmpeg.ShellUtils.ShellCallback;
...
public class DownloadsService extends Service {
				...
				case DownloadManager.STATUS_SUCCESSFUL:
					...
					if (copyEnabled && !removeVideo) {
						...
						try {
							...
							Toast.makeText(context,"Easy Video Downloader: " + context.getString(R.string.copy_progress), Toast.LENGTH_LONG).show();
							...
							if (ShareActivity.dm.remove(id) == 0) {
								Toast.makeText(context, "Easy Video Downloader: " + getString(R.string.download_remove_failed), Toast.LENGTH_LONG).show();
						...
						new Thread(new Runnable() {
							...
							public void run() {
							    ...
							    try {
							    	...
							    	try {
										Toast.makeText(context,"Easy Video Downloader: " + text, Toast.LENGTH_LONG).show();
									} catch (NullPointerException e) {
	...
	public void addId3Tags(File src) throws IOException, ID3WriteException {
        ...
        if (src_set == null) {
	        ...
	        MusicMetadata meta = new MusicMetadata("Easy Video Downloader");
	        meta.setAlbum("Easy Video Downloader Extracted Audio");
	        meta.setArtist("Easy Video Downloader");
	...
	public void setNotificationForAudioJobError() {
		...
		Toast.makeText(nContext,  "Easy Video Downloader: " + text, Toast.LENGTH_LONG).show();
		aBuilder.setContentText(text);
	}
